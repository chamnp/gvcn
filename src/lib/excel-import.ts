...
export async function parseTeacherExcelFile(file: File): Promise<TeacherProfile[]> {
  ...
  for (let i = bestHeaderRowIndex + 1; i < rawRows.length; i++) {
    ...
    results.push({
      ...
      role,
      title: title || (role === 'ADMIN' ? 'Quản Trị Viên' : role === 'ADMIN_TEACHER' ? 'Admin kiêm GVCN' : 'Giáo viên Chủ nhiệm'),
      department: department || (role === 'ADMIN' ? 'Quản Trị Hệ Thống' : 'Tổ Chuyên môn'),
      assignedClassName,
      phone,
      isActive: true,
