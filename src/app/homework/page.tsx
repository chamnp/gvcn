...
export default function HomeworkPage() {
  ...
  const [activeTab, setActiveTab] = useState<'HOMEWORK' | 'RESULTS' | 'SUBJECTS'>('HOMEWORK');
  const [viewingQuizSubmission, setViewingQuizSubmission] = useState<QuizSubmission | null>(null);
  const [viewingQuizHomework, setViewingQuizHomework] = useState<HomeworkAssignment | null>(null);
  ...
  const handleExportQuizResultsExcel = () => {
    if (quizSubmissions.length === 0) {
      toast.error('Chưa có kết quả làm bài nào để xuất file!');
      return;
    }
    ...
    quizSubmissions.forEach((sub, idx) => {
      ...
      data.push([
        ...
        sub.studentName,
        st?.studentCode || '',
        hw?.title || 'Bài tập trắc nghiệm',
        sub.score,
        sub.correctCount,
        sub.totalCount,
        Math.round((sub.timeSpentSeconds || 0) / 60),
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'KetQuaTracNghiem');
    XLSX.writeFile(wb, `KetQuaTracNghiem_${classInfo.name}_${getLocalDateString()}.xlsx`);
    toast.success('Đã xuất file Excel kết quả bài trắc nghiệm!');
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center space-x-2 border-b border-slate-200 pb-2 overflow-x-auto no-scrollbar">
        ...
        >
          ...
          <span className="hidden sm:inline">Kết Quả Bài Trắc Nghiệm ({quizSubmissions.length})</span>
          <span className="sm:hidden">Kết Quả TN ({quizSubmissions.length})</span>
        </button>
        ...
        <Link
          href="/matrix-exam"
          className="shrink-0 inline-flex items-center space-x-1.5 text-xs font-bold text-indigo-700 hover:text-indigo-900 bg-indigo-50 hover:bg-indigo-100 px-3 py-2 rounded-xl border border-indigo-200 transition-colors"
        >
          <BookOpen className="w-3.5 h-3.5 text-indigo-600" />
          <span>Kho Đề Thi TT27 →</span>
        </Link>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-xs space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div>
                <h2 className="text-base font-bold text-slate-900 flex items-center gap-2">
                  ...
                  <span>Bảng Thống Kê & Kết Quả Bài Trắc Nghiệm Trực Tuyến</span>
                </h2>
                <p className="text-xs text-slate-500 mt-0.5">
                  Theo dõi kết quả nộp bài trực tiếp của học sinh lớp {classInfo.name}, xem chi tiết bài làm và xuất bảng điểm Excel.
                </p>
              </div>
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="inline-flex items-center space-x-1.5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs px-3.5 py-2 rounded-xl shadow-xs transition-colors cursor-pointer"
                >
            ...
            {quizSubmissions.length > 0 && (
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div className="p-3.5 bg-indigo-50 border border-indigo-100 rounded-2xl">
                  <span className="text-[11px] font-bold text-indigo-600 uppercase">Tổng lượt nộp</span>
                  <div className="text-xl font-black text-indigo-950 mt-0.5">
                    {quizSubmissions.length} bài thi
                  </div>
                </div>

                <div className="p-3.5 bg-emerald-50 border border-emerald-100 rounded-2xl">
                  <span className="text-[11px] font-bold text-emerald-600 uppercase">Điểm trung bình</span>
                ...
                <div className="p-3.5 bg-amber-50 border border-amber-100 rounded-2xl">
                  <span className="text-[11px] font-bold text-amber-600 uppercase">Đạt Điểm 9 - 10</span>
                  <div className="text-xl font-black text-amber-950 mt-0.5">
                    {quizSubmissions.filter((s) => s.score >= 9).length} học sinh
                  </div>
                </div>
              </div>
            )}
            ...
            {quizSubmissions.length === 0 ? (
              <div className="text-center py-12 space-y-3">
                <div className="w-14 h-14 rounded-full bg-slate-50 text-slate-400 flex items-center justify-center mx-auto text-2xl">
                  📝
                </div>
                <h3 className="font-bold text-sm text-slate-700">Chưa có học sinh nào nộp bài trắc nghiệm</h3>
                <p className="text-xs text-slate-500 max-w-sm mx-auto">
                  Khi học sinh vào trang lớp <strong>/hw/{classInfo.name.toLowerCase()}</strong> và bấm làm bài, kết quả và điểm số tự động sẽ xuất hiện ngay tại đây.
                </p>
              </div>
            ) : (
                ...
                <table className="w-full text-left text-xs min-w-[700px]">
                  <thead className="bg-slate-50 text-slate-700 font-bold border-b border-slate-200">
                    <tr>
                      ...
                      <th className="p-3">Bài Trắc Nghiệm</th>
                    ...
                    {quizSubmissions.map((sub, idx) => {
                      ...
                      return (
                        <tr key={sub.id} className="hover:bg-slate-50 transition-colors">
                          ...
                          <td className="p-3">
                            <div className="font-bold text-slate-800">{hw?.title || 'Bài tập trắc nghiệm'}</div>
                            <div className="text-[11px] text-slate-500">{hw?.subjectName}</div>
                          </td>
                          <td className="p-3 text-center">
                            <span className={`inline-block font-black px-2.5 py-1 rounded-xl text-xs ${scoreColor}`}>
                              {sub.score} / 10
                            </span>
                          </td>
                            ...
                            <div className="flex items-center justify-end space-x-1.5">
                              {hw && (
                                <button
                                  type="button"
                                  onClick={() => {
                                    setViewingQuizHomework(hw);
                                    setViewingQuizSubmission(sub);
                                  }}
                                  className="inline-flex items-center space-x-1 text-xs font-bold text-indigo-600 bg-indigo-50 hover:bg-indigo-100 px-2.5 py-1.5 rounded-lg border border-indigo-200 transition-colors"
                                >
                                  <Eye className="w-3.5 h-3.5" />
                                  <span>Xem Bài</span>
                                </button>
                              ...
                              <button
                                type="button"
                                onClick={() => {
                                  if (confirm(`Bạn có chắc chắn muốn xóa bài làm của em ${sub.studentName}?`)) {
                                    deleteQuizSubmission(sub.id);
      ...
      {/* TAB 3: CUSTOM SUBJECTS MANAGEMENT */}
      {activeTab === 'SUBJECTS' && (
        <div className="space-y-4">
          <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-xs space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
      ...
      {/* MODAL: VIEW QUIZ SUBMISSION DETAILS */}
      {viewingQuizHomework && (
        <StudentQuizModal
          isOpen={Boolean(viewingQuizSubmission)}
          onClose={() => {
            setViewingQuizSubmission(null);
            setViewingQuizHomework(null);
          }}
          homework={viewingQuizHomework}
          students={students}
          initialSubmission={viewingQuizSubmission || undefined}
        />
      )}
    </div>
  );
